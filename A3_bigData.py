...
# Título principal
st.title("📊 Análise da COVID-19 na Bahia")
st.markdown("---")
...
def load_despesas_data():
    ...
    with st.spinner("Carregando dados de despesas..."):
        ...
        despesas = despesas[despesas["Ano"] >= 2020].copy()
...
try:
    ...
    doses = load_vacinas_data()
    gastos = load_despesas_data()
    ...
    with tab1:
        ...
        fig.add_trace(
            go.Bar(x=tabela["ANO"], y=tabela["OBITOS"], name="Óbitos", marker_color="#E63946"),
            secondary_y=False
        )
        
        fig.add_trace(
            go.Scatter(x=tabela["ANO"], y=tabela["DOSES"]/1_000_000, name="Doses (milhões)",
                      line=dict(color="#2A9D8F", width=2)),
            secondary_y=True
        )
        
        fig.add_trace(
            go.Scatter(x=tabela["ANO"], y=tabela["GASTO_MILHOES"], name="Gasto (R$ milhões)",
                      line=dict(color="#457B9D", width=2, dash="dash")),
            secondary_y=True
        )
        
        fig.update_layout(
            title="Vacinação, Investimento e Mortes na Bahia",
            hovermode="x unified",
            height=500,
            legend=dict(orientation="h", yanchor="bottom", y=1.02)
        )
        
        fig.update_yaxes(title_text="Óbitos", secondary_y=False)
        fig.update_yaxes(title_text="Doses (mi) / Gasto (R$ mi)", secondary_y=True)
        
        st.plotly_chart(fig, use_container_width=True)
    
    # TAB 2 - INVESTIMENTO
    with tab2:
        st.header("Investimento em Saúde")
        
        col1, col2 = st.columns(2)
        
        with col1:
            fig = px.line(tabela, x="ANO", y="GASTO_MILHOES", 
                         title="Evolução do Investimento",
                         markers=True,
                         labels={"GASTO_MILHOES": "R$ milhões", "ANO": "Ano"})
            fig.update_traces(line_color="#2A9D8F", marker_color="#2A9D8F")
            st.plotly_chart(fig, use_container_width=True)
        
        with col2:
            fig = px.bar(tabela, x="ANO", y="GASTO_MILHOES",
                        title="Gasto por Ano",
                        text="GASTO_MILHOES",
                        labels={"GASTO_MILHOES": "R$ milhões", "ANO": "Ano"})
            fig.update_traces(marker_color="#2A9D8F", texttemplate='R$ %{text:.1f}M', textposition='outside')
            st.plotly_chart(fig, use_container_width=True)
        ...
        with col4:
            st.metric("Total Investido", f"R$ {tabela['GASTO_MILHOES'].sum():.1f}M")
        with col5:
            st.metric("Média Anual", f"R$ {tabela['GASTO_MILHOES'].mean():.1f}M")
        with col6:
            maior_ano = tabela.loc[tabela['GASTO_MILHOES'].idxmax(), 'ANO']
            st.metric("Maior Investimento", f"R$ {tabela['GASTO_MILHOES'].max():.1f}M", f"Ano {maior_ano}")
    ...
    with tab3:
        ...
        sexo_counts = covid["SEXO"].value_counts().reset_index()
        sexo_counts.columns = ["SEXO", "COUNT"]
        
        fig = px.pie(sexo_counts, values="COUNT", names="SEXO",
    ...
    with tab4:
        ...
        # Botão de download
        csv = tabela.to_csv(index=False)
        st.download_button(
            label="📥 Baixar CSV",
            data=csv,
            file_name="dados_covid_bahia.csv",
            mime="text/csv"
        )
        
        # Mostrar dados detalhados
        with st.expander("Ver dados detalhados de óbitos"):
            st.dataframe(covid.head(100))
